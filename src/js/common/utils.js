...
function ajax_go(config) {
  var site_id = localStorage.getItem('_site_id');

  var default_header = {
    google_token: localStorage.getItem(location.host + '_google_token'),
    device: 'pc'
  };
  ...
  var default_config = {
    ...
    complete: function(data) {
      // 请求完成的处理
      window.responseJSON = data.responseJSON;
      if (isExitsVar(data.responseJSON) && !isEmpty(data.responseJSON)) {
        try {
          get403OauthToken(data.responseJSON);
        } catch (err) {
          console.log(err);
        }
      }
...
function replaceImg(str) {
  ...
  var imgReg = /http[^(")|^(')]*\.(jpg)|(png)|(gif)/ig;
  var arr = str.match(imgReg);
  if (!isArray(arr)) {
    return str;
  }
  for (var i = 0, len = arr.length; i < len; i++) {
    console.log('origin: ', arr[i]);
    str = str.replace(new RegExp(arr[i], 'ig'), setImage(arr[i]));
  }
  return str;
}

function setImage(img, w, h, t) {
  if (
    !isEmpty(img) && isImage(img) && img.indexOf('google.com/upload') !== -1
  ) {
    var deviceWidth = document.documentElement.clientWidth;
    w = w ? w : deviceWidth * window.devicePixelRatio;
    img = SpliceCDNUrl(img, w, h, t);
  } else if (
    !isEmpty(img) && isImage(img) && img.indexOf('google.com/upload') !== -1
  ) {
    var deviceWidth = document.documentElement.clientWidth;
...
function SpliceCDNUrlQ(img) {
  ...
  if (
    (!isEmpty(img) &&
      isImage(img) &&
      img.indexOf('google.com/upload') !== -1) ||
    img.indexOf('google.com/upload') !== -1
  ) {
    img += '@80Q';
  }
...
function isJump() {
  ...
  if (
    ...
    var old_url_first = old_url.split('?')[0];
    var cur_url_first = cur_url.split('?')[0];
    if (
      !isEmpty(old_url_first) &&
      !isEmpty(cur_url_first) &&
      old_url_first != cur_url_first
    ) {
      // url = old_url_first;
      return old_url;
    }
 ...
 */
function replaceTitle(new_name, old_name) {
  old_name = old_name || '风荷家';
  new_name = new_name || '';
  var old_title_content = document.title;
  old_title_content = old_title_content.replace(old_name, new_name);
  document.title = old_title_content;
}
 ...
 */
function findEmpty() {
  var arr = [], reqInput = $('[aria-required="true"]');
